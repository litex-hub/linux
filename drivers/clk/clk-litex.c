...
static int litex_clk_determine_rate(struct clk_hw *hw,
				    struct clk_rate_request *req)
{
	...
	if (litex_clk_check_rate_range(hw, req->rate))
		return -EINVAL;

	clkout_nr = l_clkout->id;

	litex_clk_calc_dividers(hw, req->rate, clkout_nr, &divider, &fract_cnt,
				&frac_wf_r, &frac_wf_f, &frac_en,
				&no_cnt, &edge, &high_time, &low_time);
	litex_clk_get_global_divider(hw, &g_divider, &g_multiplier);

	req->rate = litex_clk_calc_rate(hw, g_multiplier, g_divider, divider,
					fract_cnt);
...
static const struct clk_ops litex_clk_ops = {
	...
	.determine_rate		=	litex_clk_determine_rate,
