...
static int send_cmd(struct litex_mmc_host *host, u8 cmd, u32 arg,
	...
	if (response_len != SDCARD_CTRL_RESPONSE_NONE) {
		litex_readbuf32(host->sdcore + LITEX_MMC_SDCORE_CMDRSP_OFF,
				  host->resp, 4);
	}
	...
	n = jiffies + 2 * HZ;
	while (litex_read8(xfer_done) != 0x01)
...
static void litex_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (data) {
		...
		litex_write32(host->sdcore + LITEX_MMC_SDCORE_BLKLEN_OFF,
				 data->blksz);
...
static int litex_mmc_probe(struct platform_device *pdev)
{
	...
	litex_write64(host->sdreader + LITEX_MMC_SDBLK2MEM_BASE_OFF, 0);
	litex_write32(host->sdreader + LITEX_MMC_SDBLK2MEM_LEN_OFF, 0);

	litex_write8(host->sdwriter + LITEX_MMC_SDMEM2BLK_ENA_OFF, 0);
	litex_write64(host->sdwriter + LITEX_MMC_SDMEM2BLK_BASE_OFF, 0);
	litex_write32(host->sdwriter + LITEX_MMC_SDMEM2BLK_LEN_OFF, 0);
	return 0;
